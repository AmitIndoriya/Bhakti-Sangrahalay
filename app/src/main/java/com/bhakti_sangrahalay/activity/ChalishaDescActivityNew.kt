...
class ChalishaDescActivityNew : BaseActivity() {
    ...
    fun attachViewModel() {
        ...
        if (MyApplication.applicationContext().dataHoler.chalishaListSize == 0) {
            viewModel.readResourceFile()
        }
        // MyApplication.applicationContext().dataHoler.hashMapMutableLiveData.observe(this, Observer { hashMap -> setUpViewPager(hashMap, viewModel.getImages()) })
