...
class AartiDescFragmentNew : Fragment(), View.OnClickListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        aartiBean = MyApplication.applicationContext().dataHoler.hashMap[fragId] as AartiBean3
        ...
        playBtn = view.findViewById<ImageButton>(R.id.play_button)
        pauseBtn = view.findViewById<ImageButton>(R.id.pause_button)
    ...
    private fun showProgressBar(progress: Int) {
        if (aartiBean.isDownLoading) {
            if (progress in 1..99) {
                ...
                MyApplication.applicationContext().dataHoler.updateHashMap(fragId, aartiBean)
            } else if (progress == -1) {
                progressBar2.visibility = View.GONE
                progressBar1.visibility = View.GONE
            } else {
    ...
    fun startDownLoad() {
        ...
        MyApplication.applicationContext().dataHoler.updateHashMap(id, aartiBean)
        showProgressBar(aartiBean.progressStatus)
    ...
    fun showPlayOrPauseBtnVisibality(musicPlayStausModel: MusicPlayStausModel) {
    ...
    fun setSeekbarProgress(musicSeekBarProgress: MusicSeekBarProgress) {
